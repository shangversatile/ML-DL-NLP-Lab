...
| 自回归模型 | 按历史变量分解联合分布的生成模型 | $p(x_{1:T})=\prod_{t=1}^{T} p(x_t \mid x_{<t})$ | 概率模型族 |
| 语言模型 | 对词元序列建模 | $P(x_1,\dots,x_T)$ 或下一个词元条件分布 | 模型族 / 训练目标 |
...
| 能量模型 | 用能量函数描述未归一化概率 | $p_\theta(x) \propto \exp(-E_\theta(x))$，配分函数 | 生成 / 概率模型族 |
